...
internal class MarkupButton
{
    ...
    public string GenerateFunc()
    {
        ...
        sb.AppendLine(MarkupButtonLiterals.DecoratorFunction.Replace("NAME", ButtonProperty.Name));
        sb.AppendLine(MarkupButtonLiterals.HeadFunction.Replace("FUNC", ButtonProperty.Name));
    ...
    private string GenerateSendMessageText(bool isInline = true)
    {
        ...
        if (isInline)
            ...
            sb.AppendLine("\t" + InlineButtonLiterals.SendText.Replace("call.message.chat.id", "message.chat.id").Replace("name", ButtonProperty.Text));
        return sb.ToString();
    }
